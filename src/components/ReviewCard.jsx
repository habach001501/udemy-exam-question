...
const ReviewCard = ({
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center mb-6 text-text-muted text-sm uppercase tracking-wider">
                <div className="flex items-center gap-3">
                    <span>Question {index + 1}</span>
                    {question.isNew && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 text-black normal-case">
                            <i className="fa-solid fa-sparkles"></i> NEW
                        </span>
                    )}
                    {question.isAlwaysIncorrect && !question.isNew && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold rounded-full bg-gradient-to-r from-red-500 to-rose-600 text-white normal-case">
                            <i className="fa-solid fa-exclamation-triangle"></i> ATTENTION
                        </span>
                    )}
                    {question.source && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium rounded-full bg-white/10 text-text-muted border border-white/10 normal-case">
                ...
                {question.prompt.answers.map((ansHtml, idx) => {
                    ...
                    return (
                        <div
                            ...
                            onClick={() =>
                                handleOptionClick(letter, question.correct_response.length > 1)
                            }
                        >
                            <div
                                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 text-sm font-bold ${markerClass}`}
                            >
                                {letter.toUpperCase()}
                            </div>
                            <div
                                className="flex-1 [&>p]:m-0"
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                                {showYourAnswerLabel && (
                                    <span
                                        className={`text-[10px] font-bold px-2 py-0.5 rounded ${isCorrect ? "bg-success/20 text-success" : "bg-danger/20 text-danger"}`}
                                    >
                                        Your Answer
                                    </span>
                                )}
            ...
            {(!interactiveMode || revealed) && (
                <div className="mt-8 p-6 bg-green-500/10 border border-green-500/20 rounded-lg text-green-100 animate-fade-in">
                    ...
                    <div
                        className="[&>p]:mb-2 [&>img]:max-w-full [&>img]:rounded-lg break-words whitespace-pre-wrap text-sm"
                        dangerouslySetInnerHTML={{ __html: question.prompt.explanation }}
                    ></div>
                </div>
            )}
