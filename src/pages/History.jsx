            ...
            <div className="h-[calc(100vh-80px)] w-[95vw] mx-auto flex flex-col gap-4">
                ...
                <div className="px-6 py-4 flex items-center justify-between bg-bg-card/80 backdrop-blur-md border border-white/10 rounded-2xl shadow-xl shrink-0 mt-2">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <div className="text-sm text-text-muted mt-1 flex items-center gap-3">
                                ...
                                <span className={`flex items-center gap-1.5 font-semibold ${selectedAttempt.percent >= 75 ? 'text-success' : 'text-danger'}`}>
                                    <i className={`fa-solid ${selectedAttempt.percent >= 75 ? 'fa-check' : 'fa-xmark'}`}></i>
                    ...
                    >
                        ...
                        <span className="text-sm font-medium text-text-muted group-hover:text-white">Back to History</span>
                    </button>
                </div>
            ...
            <div className="bg-bg-card/80 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl mb-8 relative overflow-hidden group">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <h1 className="text-3xl font-black text-white tracking-tight mb-2">My History</h1>
                ...
                {history.length === 0 ? (
                    ...
                    history.map((h, idx) => (
                        ...
                        >
                            ...
                            <div className={`absolute top-0 left-0 w-1 h-full ${h.percent >= 75 ? 'bg-success' : 'bg-danger'} opacity-50 group-hover:opacity-100 transition-opacity`}></div>

                            <div className="flex flex-wrap items-center gap-4 p-5 pl-7">
                                {/* Date and ID Info */}
                                <div className="min-w-[140px]">
                                    <div className="font-bold text-white">{new Date(h.date).toLocaleDateString()}</div>
                                    <div className="text-xs text-text-muted mt-0.5">{new Date(h.date).toLocaleTimeString()}</div>
                                </div>
                                ...
                                <div className="flex-1 flex items-center justify-center gap-6">
                                    <div className="text-center">
                                        <div className={`text-2xl font-black ${h.percent >= 75 ? 'text-success' : 'text-danger'}`}>
                                            {h.percent}%
                                        </div>
                                    ...
                                    <div className="flex gap-1 text-xs font-mono bg-black/20 p-1.5 rounded-lg">
                                        <div className="px-2 py-0.5 rounded bg-success/10 text-success border border-success/20" title="Correct">
                                            {h.score || 0} C
                                        </div>
                                        <div className="px-2 py-0.5 rounded bg-danger/10 text-danger border border-danger/20" title="Incorrect">
                                            {h.incorrect || 0} W
                                        </div>
                                        <div className="px-2 py-0.5 rounded bg-warning/10 text-warning border border-warning/20" title="Skipped">
                                            {h.unanswered || 0} S
                                        </div>
                                    </div>
                                </div>
