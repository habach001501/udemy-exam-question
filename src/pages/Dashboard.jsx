...
const Dashboard = () => {
    ...
    if (loading) return <div className="max-w-[1200px] mx-auto px-4 py-8">Loading resources...</div>;

    const course = state.currentCourse || courses.find(c => c.id === courseId);

    return (
        <div className="max-w-[1200px] mx-auto px-4 flex flex-col h-[90vh] pb-0 animate-fade-in">
            <h1 className="text-4xl sm:text-5xl mb-8 font-bold text-center bg-gradient-to-r from-white to-text-muted bg-clip-text text-transparent" id="course-title">
                {course?.title}
            </h1>
            ...
            {view === 'exam-setup' && (
                <div className="animate-fade-in">
                    <h2
                        ...
                        className="text-2xl font-bold mb-6 cursor-pointer flex items-center gap-2 hover:text-primary transition-colors"
                    >
                        <i className="fa-solid fa-arrow-left"></i> Configure Exam
                    </h2>
                    <div className="bg-bg-card border border-white/5 rounded-xl p-8 cursor-default">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                <label className="block mb-2 font-semibold text-text-main">Number of Questions</label>
                                <input
                                    type="number"
                                    value={examConfig.count}
                                    onChange={(e) => setExamConfig({ ...examConfig, count: parseInt(e.target.value) })}
                                    min="1" max="500"
                                    className="w-full p-4 bg-bg-dark text-white border border-white/10 rounded-lg focus:outline-none focus:border-primary transition-colors"
                                />
                            </div>
                            <div>
                                <label className="block mb-2 font-semibold text-text-main">Time Limit (Minutes)</label>
                                <input
                                    type="number"
                                    value={examConfig.time}
                                    onChange={(e) => setExamConfig({ ...examConfig, time: parseInt(e.target.value) })}
                                    min="1"
                                    className="w-full p-4 bg-bg-dark text-white border border-white/10 rounded-lg focus:outline-none focus:border-primary transition-colors"
                                />
                            </div>
                ...
                <div className="animate-fade-in">
                    ...
                    <div className="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-6">
                        {state.availableData.map((set, idx) => (
                            <div
                                key={idx}
                                className="bg-bg-card border border-white/5 rounded-xl p-8 text-center cursor-pointer hover:-translate-y-1 hover:shadow-xl hover:border-primary transition-all duration-200"
                                onClick={() => startReview(idx)}
                            >
                                <h3 className="text-xl font-bold mb-2">{set.name}</h3>
                                <p className="text-text-muted">{set.count} Questions</p>
                            </div>
                        ))}
                    </div>
                </div>
