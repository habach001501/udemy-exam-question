        ...
        <div className="max-w-[1200px] mx-auto px-4 py-8 animate-fade-in flex flex-col items-center justify-center min-h-[80vh]">
            ...
            <div className="text-center bg-bg-card p-12 rounded-2xl border border-white/5 shadow-2xl w-full max-w-lg">
                ...
                <div
                    ...
                    style={{ background: `conic-gradient(${stats.percent >= 75 ? '#10b981' : '#ef4444'} ${stats.percent * 3.6}deg, #334155 0)` }}
                >
                    <div className="absolute w-[170px] h-[170px] bg-bg-dark rounded-full flex flex-col items-center justify-center z-10">
                        <span className={`text-5xl font-extrabold ${stats.percent >= 75 ? 'text-success' : 'text-danger'}`}>{stats.percent}%</span>
                        <span className="text-text-muted text-sm uppercase tracking-wide mt-1">Score</span>
                    </div>
                </div>

                <h2 className={`text-3xl font-bold mb-8 ${stats.percent >= 75 ? 'text-success' : 'text-danger'}`}>
                    {stats.percent >= 75 ? 'PASSED! Great job.' : 'FAILED. Keep practicing.'}
                </h2>
