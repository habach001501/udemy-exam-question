export const neal_1 = {
  ...
  results: [
    ...
    },
    {
      ...
      question_plain:
        "A company manages several legacy applications that all generate different log formats. The logs need to be standardized so they can be queried and analyzed. A DevOps engineer needs a solution for standardizing the log formats before writing them toan Amazon S3 bucket.How can this requirement be met at the LOWEST cost?",
      related_lectures: [],
    },
    {
      _class: "assessment",
      ...
      prompt: {
        question:
          "<p>A company runs many different workloads across hundreds of Amazon EC2 instances. The DevOps team requires that all instances have standard configurations. These configurations include standard logging, metrics, security assessments, and weekly patching.</p><p>Which combination of actions meets these requirements with the most operational efficiency? (Select TWO.)</p>",
        relatedLectureIds: "",
        feedbacks: ["", "", "", "", ""],
        explanation:
    ...
    },
    {
      ...
      prompt: {
        question:
          "<p>A DevOps engineer launched an Amazon EC2 instance in an Amazon VPC. The instance must download an object from a restricted Amazon S3 bucket. When trying to download the object, a 403 Access Denied error was received.</p><p>What are two possible causes for this error? (Select TWO.)</p>",
        relatedLectureIds: "",
        feedbacks: ["", "", "", "", ""],
        explanation:
          '<p>The most likely cause of this error message is that either the S3 bucket policy or IAM role configuration has an error or simply does not grant the required permissions. The DevOps engineer should review the bucket policy or associated IAM user policies for any statements that might be denying access.</p><p>The engineer should also verify that the requests to the bucket meet any conditions in the bucket policy or IAM policies, checking for any incorrect deny statements, missing actions, or incorrect spacing in the policy.</p><p><strong>CORRECT: </strong>"The bucket policy does not grant permission" is a correct answer (as explained above.)</p><p><strong>CORRECT: </strong>"There is an issue with the IAM role configuration" is also a correct answer (as explained above.)</p><p><strong>INCORRECT:</strong> "Default encryption is enabled on the S3 bucket" is incorrect.</p><p>This would not affect access to the object as the object would be automatically decrypted by S3.</p><p><strong>INCORRECT:</strong> "The object has been moved to Amazon Glacier" is incorrect.</p><p>With Glacier an AccessDeniedException error would be received if permission is not granted.</p><p><strong>INCORRECT:</strong> "S3 versioning is enabled on the S3 bucket" is incorrect.</p><p>Versioning does not affect access permissions to an object.</p><p><strong>References:</strong></p><p><a href="https://aws.amazon.com/premiumsupport/knowledge-center/s3-troubleshoot-403/">https://aws.amazon.com/premiumsupport/knowledge-center/s3-troubleshoot-403/</a></p><p><strong>Save time with our AWS cheat sheets:</strong></p><p><a href="https://digitalcloud.training/amazon-s3-and-glacier/">https://digitalcloud.training/amazon-s3-and-glacier/</a></p>',
        answers: [
      ...
      section: "AWS Storage",
      question_plain:
        "A DevOps engineer launched an Amazon EC2 instance in an Amazon VPC. The instance must download an object from a restricted Amazon S3 bucket. When trying to download the object, a 403 Access Denied error was received.What are two possible causes for this error? (Select TWO.)",
      related_lectures: [],
    },
    {
      _class: "assessment",
      ...
      prompt: {
        question:
          "<p>An application runs on Amazon EC2 instances behind an Application Load Balancer (ALB). Users on the internet have reported performance issues with the application. A DevOps engineer must investigate the reports and identify the processing latencies for requests.</p><p>How can the engineer obtain this information?</p>",
        relatedLectureIds: "",
        feedbacks: ["", "", "", ""],
        explanation:
        ...
        answers: [
          "<p>Use AWS CIoudTrail to identify API requests.</p>",
          "<p>Enable access logs on the load balancer.</p>",
          "<p>Install the Amazon CloudWatch agent.</p>",
          "<p>Review Amazon CloudWatch metrics.</p>",
        ],
      },
      correct_response: ["b"],
      section: "AWS Networking & Content Delivery",
    ...
    },
    {
      ...
      section: "AWS Management & Governance",
      question_plain:
        "A security review has identified that an AWS CodeBuild project is downloading a database population script from an Amazon S3 bucket using an unauthenticated request. The security team has prohibited unauthenticated requests to S3 buckets for this project.How can this issue be resolved in the MOST secure manner?",
      related_lectures: [],
    },
    {
      _class: "assessment",
    ...
    },
    {
      ...
      prompt: {
        ...
        answers: [
          "<p>Create an AWS Lambda function that evaluates the deployed resources in the account and evaluates them against resource limits on the account. Create an Amazon EventBridge rule to run the Lambda function regularly and configure an Amazon SNS topic that notifies the operations manager.</p>",
          "<p>Create an AWS Lambda function that refreshes AWS Personal Health Dashboard checks and use an Amazon EventBridge rule to run the Lambda function regularly. Create another EventBridge rule with an event pattern matching Trusted Advisor checks and configure an Amazon SNS topic that notifies the operations manager.</p>",
          "<p>Create an AWS Config custom rule that runs regularly and checks the AWS service limit status and sends notifications to an Amazon SNS topic. Deploy an AWS Lambda function that notifies the operations manager and subscribe the Lambda function to the SNS topic.</p>",
          "<p>Create an AWS Lambda function that refreshes AWS Trusted Advisor checks and use an Amazon EventBridge rule to run the Lambda function regularly. Create another EventBridge rule with an event pattern matching Trusted Advisor checks and configure an Amazon SNS topic that notifies the operations manager.</p>",
        ],
      },
      correct_response: ["d"],
      section: "AWS Security, Identity, & Compliance",
