export const neal_5 = {
  ...
  results: [
    ...
    },
    {
      ...
      section: "AWS Compute",
      question_plain:
        "A DevOps engineer needs a managed environment for running a Node.js application. The infrastructure should support load balancing and auto scaling. The application will require a managed relational database, and data should be stored persistently and protected from accidental deletion. The solution should minimize ongoing operational effort.Which actions should the engineer take to meet these requirements? (Select TWO.)",
      related_lectures: [],
    },
    {
      _class: "assessment",
    ...
    },
    {
      ...
      prompt: {
        question:
          "<p>The security team at a company requires a solution to identify activities that indicate that Amazon EC2 instances have been compromised. The solution should notify them by email if issues are discovered.</p><p>Which solution will meet these requirements?</p>",
        relatedLectureIds: "",
        feedbacks: ["", "", "", ""],
        explanation:
    ...
    },
    {
      ...
      prompt: {
        question:
          "<p>To improve security, a company plans to use AWS Systems Manager Session Manager to manage EC2 instances instead of using key pairs. The company also requires that access to Session Manager goes across private networks only.</p><p>Which combinations of actions will accomplish this? (Select TWO.)</p>",
        relatedLectureIds: "",
        feedbacks: ["", "", "", "", ""],
        explanation:
      ...
      section: "AWS Management & Governance",
      question_plain:
        "To improve security, a company plans to use AWS Systems Manager Session Manager to manage EC2 instances instead of using key pairs. The company also requires that access to Session Manager goes across private networks only.Which combinations of actions will accomplish this? (Select TWO.)",
      related_lectures: [],
    },
    {
      _class: "assessment",
      ...
      section: "AWS Management & Governance",
      question_plain:
        "A DevOps Engineer is responsible for a containerized application deployed on Amazon ECS. The application retrieves JSON-formatted configuration files at runtime. Each file is approximately 3 KB in size, and the number of configuration files is expected to grow significantly with increased customer onboarding.The following requirements must be met:· The configuration files must be updateable dynamically without requiring container restarts.· A change history must be maintained for auditing and rollback purposes.· The solution must scale with minimal operational effort and use managed AWS services wherever possible.What is the BEST solution for managing and loading the configuration files?",
      related_lectures: [],
    },
    {
      _class: "assessment",
    ...
    },
    {
      ...
      prompt: {
        question:
          "<p>An application is being deployed using an AWS CodePipeline pipeline. The pipeline includes an AWS CodeBuild stage which downloads source code from AWS CodeCommit, pulls data from an S3 bucket, and builds and tests the application before deployment.</p><p>A DevOps engineer has discovered that the S3 data is not being successfully downloaded due to a permissions issue.</p><p>How can the permissions be assigned to CodeBuild in the MOST secure manner?</p>",
        relatedLectureIds: "",
        feedbacks: ["", "", "", ""],
        explanation:
