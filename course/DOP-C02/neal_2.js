  ...
  results: [
    ...
    },
    {
      ...
      prompt: {
        ...
        answers: [
          "<p>Create an AWS Lambda function that uses Amazon SES to send the email notification. Add an Amazon API Gateway API to invoke the function. Call the API from the client side when login confirmation is received.</p>",
          "<p>Create an AWS Lambda function that uses Amazon SES to send the email notification. Add an Amazon Cognito post authentication Lambda trigger for the function.</p>",
          "<p>Create an AWS Lambda function that uses Amazon SES to send the email notification. Create an Amazon CloudWatch Logs log subscription filter to invoke the function based on the login status.</p>",
          "<p>Configure Amazon Cognito to stream all logs to Amazon Kinesis Data Firehose. Create an AWS Lambda function to process the streamed logs and to send the email notification based on the login status of each user.</p>",
        ],
      },
      correct_response: ["b"],
      section: "AWS Security, Identity, & Compliance",
    ...
    },
    {
      ...
      prompt: {
        question:
          "<p>An organization is running containerized applications across Amazon EKS, Amazon ECS, and on-premises clusters. Due to some recent issues that caused outages, a solution is required to track container performance and system health, detect errors. The solution should enable collection and aggregation of time-series metrics from all container services for monitoring and analytics.</p><p>Which combination of services can the organization use?</p>",
        relatedLectureIds: "",
        feedbacks: ["", "", "", ""],
        explanation:
    ...
    },
    {
      ...
      prompt: {
        question:
          "<p>A company requires an extremely high performance in-memory database for an application. The database used should store data durably across multiple availability zones. The application requires that the database support strong consistency and can scale seamlessly to many terabytes in size.</p><p>Which database should the company use for this application?</p>",
        relatedLectureIds: "",
        feedbacks: ["", "", "", ""],
        explanation:
    ...
    },
    {
      ...
      prompt: {
        question:
          "<p>A financial company has applications hosted in multiple AWS accounts which are managed by AWS organizations. A security audit was conducted to address any potential security breaches and implement best practices. The security audit report recommended that all security related logging and security findings are collect in a centralized security account.</p><p>How can this be achieved?</p>",
        relatedLectureIds: "",
        feedbacks: ["", "", "", ""],
        explanation:
          '<p>GuardDuty analyzes and processes VPC flow log, AWS CloudTrail event log, and DNS log data sources. You don’t need to manually manage these data sources because the data is automatically leveraged and analyzed when you activate GuardDuty.</p><p>For example, GuardDuty consumes VPC Flow Log events directly from the VPC Flow Logs feature through an independent and duplicative stream of flow logs. As a result, you don’t incur any operational burden on existing workloads.</p><img src="https://img-c.udemycdn.com/redactor/raw/practice_test_question_explanation/2023-02-13_00-10-01-c38c34b5c636f2d9f839cf769fa2e58c.jpg">',
        answers: [
    ...
    },
    {
      ...
      prompt: {
        ...
        answers: [
          ...
          "<p>Create multiple Lambda layers for different versions of the code. Add the updated code to a new layer using a .zip archive. Create an alias that points to the layer containing the original code. Add the new layer to the alias and configure a weight that directs 10% of traffic to the new layer. Modify the application on EC2 to use the alias endpoint address.</p>",
        ],
      },
      correct_response: ["a"],
      section: "AWS Compute",
    ...
    },
    {
      ...
      section: "AWS Management & Governance",
      question_plain:
        "To increase security, a company plans to use AWS Systems Manager Session Manager to managed Amazon EC2 instances rather than SSH. The connectivity to Session Manager should also use a private network connection.Which configuration actions should be taken to implement this? (Select TWO.)",
      related_lectures: [],
    },
    {
      _class: "assessment",
