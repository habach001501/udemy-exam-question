  ...
  results: [
    {
      ...
      prompt: {
        ...
        answers: [
          "<p>Use AWS Secrets Manager to store sensitive data as secret values and use dynamic references in AWS CloudFormation.</p>",
          "<p>Use AWS Systems Manager Parameter Store to store sensitive data as secure strings and reference the strings using tags in the CloudFormation template.</p>",
          "<p>Use AWS Secrets Manager to encrypt the CloudFormation template at rest using an AWS KMS key.</p>",
          "<p>Use the CloudFormation NoEcho parameter property to mask any sensitive parameter values.</p>",
          "<p>Use an Amazon S3 bucket with default encryption enabled to store the AWS CloudFormation template.</p>",
        ],
      },
    ...
    },
    {
      ...
      prompt: {
        question:
          "<p>An application runs on Amazon EC2 instances in an Auto Scaling group behind an application Load Balancer (ALB). The applications runs across two Availability Zones (AZs). Recently the application has become very popular and to increase reliability of the application a DevOps engineer has configured the Auto Scaling group to launch instances across three AZs. However, instances launched in the newly added AZ are not receiving any traffic.</p><p>What is the most likely cause of this issue?</p>",
        relatedLectureIds: "",
        feedbacks: ["", "", "", ""],
        explanation:
    ...
    },
    {
      ...
      section: "AWS Management & Governance",
      question_plain:
        "A company manages both Amazon EC2 instances and on-premises servers running Linux and Windows. A DevOps engineer needs to manage patching across these environments. All patching must take place outside of business hours.Which combination of actions will meet these requirements? (Select THREE.)",
      related_lectures: [],
    },
    {
      _class: "assessment",
    ...
    },
    {
      ...
      prompt: {
        ...
        answers: [
          "<p>Use an AWS WAF IP set statement that specifies the IP addresses that needs to be allowed.</p>",
          "<p>Use an ALB geo match statement listing the countries that needs to be blocked.</p>",
          "<p>Use an ALB IP set statement that specifies the IP addresses that needs to be allow through.</p>",
          "<p>Use an AWS WAF geo match statement listing the countries that need to be blocked.</p>",
